...
#define NVRTC_CHECK_ERROR( func )                                  \
  do {                                                             \
    ...
    void optixUtil::displayBufferGL(optix::Buffer buffer, Desperado::Texture::SharedPtr tex)
    {
        ...
        GLenum internalFormat, pixelFormat, dataFormat;
        if (buffer_format == RT_FORMAT_UNSIGNED_BYTE4) {
            ...
            //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, width, height, 0, GL_RGB, GL_FLOAT, imageData);
        ...
        //     std::cout << ((float*)imageData)[i] << std::endl;
        //}

        tex->biltFromCPU(width, height, internalFormat, pixelFormat, dataFormat, 1, 0, imageData);
