...
namespace Desperado
{
	...
	public:
		...
		static void mouseMoveCallback(GLFWwindow* pGlfwWindow, double mouseX, double mouseY)
		{
			...
			if (pWindow != nullptr)
			{
				...
				event.screenOffset = {mouseX - pWindow->getLastScreenPos().x, pWindow->getLastScreenPos().y - mouseY};
				pWindow->setLastScreenPos({mouseX, mouseY});
		...
		static void mouseButtonCallback(GLFWwindow* pGlfwWindow, int button, int action, int modifiers)
		{
			...
			case GLFW_MOUSE_BUTTON_MIDDLE:
				event.type = (action == GLFW_PRESS)
					             ? MouseEvent::Type::MiddleButtonDown
					             : MouseEvent::Type::MiddleButtonUp;
				break;
			case GLFW_MOUSE_BUTTON_RIGHT:
				event.type = (action == GLFW_PRESS)
					             ? MouseEvent::Type::RightButtonDown
	...
	private:
		static inline KeyboardEvent::Key glfwToKey(int glfwKey)
		{
	...
	Window::Window(ICallbacks* pCallbacks, const Desc& desc)
		: mpCallbacks(pCallbacks)
		  ...
		  , mLastScreenPos({desc.width / 2.0f, desc.height / 2.0f})
	{
	}
	...
	Window::SharedPtr Window::create(const Desc& desc, ICallbacks* pCallbacks)
	{
		...
		glfwSetCursorPos(pGLFWWindow, pWindow->mLastScreenPos.x, pWindow->mLastScreenPos.y);
