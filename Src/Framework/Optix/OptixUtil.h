...
namespace optixUtil
{
	...
	void displayBufferPPM(
		...
		optix::Buffer buffer, // Buffer to be displayed
		bool disable_srgb_conversion = true);
	// Enables/disables srgb conversion before the image is saved. Disabled by default.            
	...
	void dlldecl displayBufferGL(
		optix::Buffer buffer, Desperado::Texture::SharedPtr tex); // Buffer to be displayed
	...
	const char* getPtxString(
		const char* sample,
		// Name of the sample, used to locate the input file. NULL = only search the common /cuda dir
		const char* filename, // Cuda C input file name
		const char** log = NULL);
	// (Optional) pointer to compiler log string. If *log == NULL there is no output. Only valid until the next getPtxString call
