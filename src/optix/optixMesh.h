...
public:
	...
	void init() {
		...
		{
			string number;
			string name = p_mesh->textures[i].type;
			...
			auto p = map_textureSampler.find(p_mesh->textures[i].id);
			if (p == map_textureSampler.end()) {
				optix::TextureSampler sampler = context->createTextureSamplerFromGLImage(p_mesh->textures[i].id, RT_TARGET_GL_TEXTURE_2D);
